...
func (r *ELASTICACHE) filter(all, filtered observer.Property) {
	logger := log.WithField("elasticache", "filter")
	logger.Debug("Starting ELASTICACHE instance filter worker")
	...
	for {
		...
		case <-stream.Changes():
			...
			logger.Debug("Finished filtering ELASTICACHE instances")
		}
	}
}
...
func (r *ELASTICACHE) matches(filter, value string) bool {
...
func (r *ELASTICACHE) startsWith(filter, value string) bool {
	for _, v := range strings.Split(filter, ",") {
